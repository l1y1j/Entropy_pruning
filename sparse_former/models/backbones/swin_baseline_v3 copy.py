...
def compute_soft_mask(scores: torch.Tensor, threshold: torch.Tensor, temperature: float = 1.0) -> torch.Tensor:
    ...
    if _mask_debug_counter % 50 == 0 and _mask_debug_counter < 500:
        print(f"DEBUG [Iter {_mask_debug_counter}]: Mask Mean = {m_mask.mean().item():.4f}, Threshold Mean = {threshold.mean().item():.4f}")
...
def compute_gate_loss(m_mask: torch.Tensor, scores: torch.Tensor, lambda_reg: float, stage_idx: int = -1, block_idx: int = -1) -> torch.Tensor:
    """计算Gate Loss
    ...
    Args:
        ...
        scores: (N,) 窗口分数
        lambda_reg: 正则化系数
        stage_idx: stage编号（用于debug打印）
        block_idx: block编号（用于debug打印）
    ...
    # Clamp info_ratio 到 [0.01, 0.99]，防止极端梯度
    info_ratio_clamped = torch.clamp(info_ratio, min=0.01, max=0.99)

    activation_ratio = m_mask.sum() / N

    # Debug打印：显示真实值 vs clamped值
    if stage_idx >= 0:
        raw_info = info_ratio.item()
        clamped_info = info_ratio_clamped.item()
        was_clamped = "CLAMPED" if abs(raw_info - clamped_info) > 1e-3 else "ok"
        print(f"[Gate-Debug] S{stage_idx}B{block_idx} | "
              f"info_ratio(raw)={raw_info:.4f}[{was_clamped}] "
              f"info_ratio(clamped)={clamped_info:.4f} | "
              f"act_ratio={activation_ratio.item():.4f} | "
              f"lambda={lambda_reg}")

    loss = (1 - info_ratio_clamped) ** 2 + lambda_reg * activation_ratio
    # Clamp loss 到 [0.0, 10.0]，防止梯度爆炸
...
class SwinBlockV3(nn.Module):
    ...
    def _forward_kl(self, x: torch.Tensor, hw_shape: tuple) -> tuple:
        ...
        kl_threshold = self.kl_predictor(stats_mean, stats_std, stats_p50, stats_max,
                                          self.stage_idx, self.block_idx)  # (B, 1)
        ...
        # ----- Step 3: 软掩码计算 -----
        kl_threshold_tiled = kl_threshold.squeeze(-1).unsqueeze(-1)  # (B, 1) -> (B, N_win)
        m_mask_kl = torch.sigmoid((kl_norm_scores - kl_threshold_tiled) / self.temperature)
        ...
        m_mask_kept = m_mask_kl.view(-1)[kl_keep_idx_flat].unsqueeze(-1).unsqueeze(-1)  # (N_kl, 1, 1)
        ...
        if self.use_learnable_gate and self.kl_predictor is not None:
            kl_loss = compute_gate_loss(m_mask_kl.view(-1), kl_norm_scores.view(-1), self.lambda_kl, self.stage_idx, self.block_idx)
        ...
        return x, full_entropy, x_windows, x_windows_new, window_scores_reshaped, (H_pad, W_pad), self.embed_dims, kl_keep_idx_flat, block_gate_loss
    ...
    def _forward_inc(self, x: torch.Tensor, hw_shape: tuple,
        ...
        stats_mean, stats_std, stats_p50, stats_max = collect_stats(scores_flat, B)
        inc_threshold = self.inc_predictor(stats_mean, stats_std, stats_p50, stats_max,
                                           self.stage_idx, self.block_idx)  # (B, 1)

        # ----- Step 2: Min-Max 归一化 -----
        scores_min = inc_scores_reshaped.min(dim=1, keepdim=True)[0]
        scores_max = inc_scores_reshaped.max(dim=1, keepdim=True)[0]
        inc_norm_scores = (inc_scores_reshaped - scores_min) / (scores_max - scores_min + 1e-8)

        # ----- Step 3: 软掩码计算 -----
        inc_threshold_tiled = inc_threshold.squeeze(-1).unsqueeze(-1)  # (B, 1) -> (B, N_win)
        m_mask_inc = torch.sigmoid((inc_norm_scores - inc_threshold_tiled) / self.temperature)
        ...
        fallback[torch.arange(B, device=x.device), inc_scores_reshaped.argmax(dim=1)] = True
        ...
        if self.use_learnable_gate and self.inc_predictor is not None:
            inc_loss = compute_gate_loss(m_mask_inc.view(-1), inc_norm_scores.view(-1), self.lambda_inc, self.stage_idx, self.block_idx)
    ...
    def _forward_kl_inc(self, x: torch.Tensor, hw_shape: tuple,
        ...
        if entropy_cache is not None and entropy_cache.shape[0] == total_windows:
            inc_scores_survived = torch.abs(cur_entropy - entropy_cache[kl_keep_idx_flat].detach())
        elif prev_aligned_entropy is not None and prev_aligned_entropy.shape[0] == total_windows:
        ...
        stats_mean, stats_std, stats_p50, stats_max = collect_stats(raw_inc_scores_flat, B)
        inc_threshold = self.inc_predictor(stats_mean, stats_std, stats_p50, stats_max,
                                           self.stage_idx, self.block_idx)  # (B, 1)
        ...
        inc_fallback[torch.arange(B, device=x.device), inc_scores_full.argmax(dim=1)] = True
        ...
        if self.use_learnable_gate:
            ...
            if self.inc_predictor is not None and inc_scores_full.numel() > 0:
                # 核心修复：直接传全尺寸张量！无效的 0.0 会在 sum 时自动隐身
                inc_loss = compute_gate_loss(m_mask_inc.view(-1), inc_norm_scores.view(-1), self.lambda_inc, self.stage_idx, self.block_idx)
                self.inc_gate_loss = inc_loss
                for b in range(B):
                    collect_threshold(inc_threshold[b].item(), self.stage_idx, self.block_idx, 'inc')
