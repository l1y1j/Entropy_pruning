...
class EntropyVisualizationHook(Hook):
    ...
    for the specified stages and blocks. Also manages KL/INC score collection
    via swin_baseline_v3 collector functions.
    ...
    def _build_enabled_collections(self, backbone):
        """从backbone的stage_config构建enabled_collections"""
        enabled = set()
        # KL 配置
        stage_config = getattr(backbone, 'stage_config', {})
        for stage_idx, cfg in stage_config.items():
            blocks = cfg.get('blocks', [])
            for block_idx in blocks:
                enabled.add((stage_idx, block_idx, 'kl'))
        # INC 配置
        inc_stage_config = getattr(backbone, 'inc_stage_config', {})
        for stage_idx, cfg in inc_stage_config.items():
            blocks = cfg.get('blocks', [])
            for block_idx in blocks:
                enabled.add((stage_idx, block_idx, 'inc'))
        return enabled
    ...
    def before_train_epoch(self, runner):
        ...
        if hasattr(model, 'backbone'):
            ...
            enabled = self._build_enabled_collections(backbone)
            register_enabled_collections(enabled)
